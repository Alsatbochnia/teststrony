...
.print-sign .signature-area { border-top:1px solid #333; width:45%; text-align:center; padding-top:5px; align-self:flex-end; }
        ...
        // Odmiana słowa "zgłoszenie" w zależności od liczby
        const pluralZgloszenia = (n) => {
            if (n === 1) return 'zgłoszenie';
            const lastDigit = n % 10, lastTwo = n % 100;
            if (lastDigit>=2 && lastDigit<=4 && !(lastTwo>=12 && lastTwo<=14)) return 'zgłoszenia';
            return 'zgłoszeń';
        };
        // === WSZYSTKIE ZGŁOSZENIA DLA DANEGO ADRESU ===
        const openAddressOrders = (address) => {
            const norm = normalizeAddress(address);
            const matches = orders.filter(o => normalizeAddress(o.device) === norm);
            document.getElementById('address-orders-title').textContent = address;
            document.getElementById('address-orders-count').textContent = `${matches.length} ${pluralZgloszenia(matches.length)} na tym adresie`;
            document.getElementById('address-orders-list').innerHTML = matches.length === 0
                ? '<div class="p-4 text-center text-gray-500">Brak zgłoszeń dla tego adresu</div>'
                : matches.slice().sort((a,b)=>b.date.localeCompare(a.date)).map(o=>`
                    <div class="clickable-row p-3 border rounded-lg cursor-pointer hover:shadow-md transition mb-2" data-id="${o.id}">
            ...
            document.querySelectorAll('#address-orders-list .clickable-row').forEach(el=>{
                el.addEventListener('click', ()=>{
                    ...
                    openOrderForEdit(orders.find(o=>o.id===el.dataset.id));
                });
            });
...
document.getElementById('orders-list').innerHTML = filteredOrders.length === 0
    ...
    : filteredOrders.map(o => `
            ...
            document.querySelectorAll('#waste-summary-list .address-link').forEach(el=>{
                el.addEventListener('click', ()=>openAddressOrders(decodeURIComponent(el.dataset.address)));
            });
            ...
            document.querySelectorAll('#vehicles-devices-list .address-link').forEach(el=>{
                if (el.dataset.address) el.addEventListener('click', ()=>openAddressOrders(decodeURIComponent(el.dataset.address)));
            });
            const vehKeys = {};
